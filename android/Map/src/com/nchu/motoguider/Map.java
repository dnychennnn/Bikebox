...
public class Map extends ListActivity
{
	...
	protected void onCreate(Bundle savedInstanceState) 
	{
		...
		*/ 
		mbts = new BTS();
		handler = new Handler(){};
		Intent getIntent = this.getIntent();           // 從Speech存的值抓過來
		...
		LatLng origin = getOriginGPS();    //取得現在位置
		super.onCreate(savedInstanceState);
		   
		String url = getDirectionsUrl(origin, dest);    //Directions API 使用一系列路點傳回多個部分的路線。
		DownloadTask downloadTask = new DownloadTask();
		downloadTask.execute(url);
		
		/* 註冊廣播 */
		IntentFilter filter = new IntentFilter("InfoService"); //IntentFliter物件可過濾intent請求
		receiver = new MyBroadcastReceiver();
		registerReceiver(receiver, filter);  //參數1是broadcast的物件，參數2是含action的IntentFilter物件。
	 }
	...
	{
		@Override
		public void onReceive(Context context, Intent intent) //Action
		{
	 ...
	 public LatLng getOriginGPS()
	 {
	     ...
	     bestProvider = locationManager.getBestProvider(criteria, true); // 如有最佳定位服務提供者，則重新取得定位參數
	     ...
	     catch (NullPointerException e)
	     {
	         ...
	         e.printStackTrace();*/
	    	 if(location==null){
				  Intent intent = new Intent(Map.this, Speech.class);
